...
import { ChevronLeftIcon, ChevronRightIcon, MinusIcon, PencilIcon, PlusIcon, ResetIcon, TrashIcon } from './icons';
...
const Toolbar: React.FC<ToolbarProps> = ({
  ...
  selectedAnnotationId, totalImages, annotatedImagesCount, annotationStats, currentImageDimensions, allImageDimensions,
  onFileSelect, onClose, onPrevious, onNext, onGoToIndex, onZoomIn, onZoomOut, onReset,
  ...
  return (
    <aside className="w-80 h-screen bg-gray-800 text-white flex flex-col p-4 border-r border-gray-700 shadow-lg shrink-0 overflow-y-auto thumbnail-scrollbar">
      ...
      <div className="mb-4 text-sm flex-shrink-0">
        <p className="font-semibold truncate" title={images[currentIndex]?.name}>{images[currentIndex]?.name || '...'}</p>
      ...
      {totalImages > 0 && (
        <div className="mb-4 pb-4 border-b border-gray-700">
          <h2 className="text-sm font-semibold text-gray-400 px-1 mb-2">Annotation Progress</h2>
          <div className="flex items-center gap-4 p-2 bg-gray-700/50 rounded-lg">
            <DonutChart progress={annotatedImagesCount / totalImages} size={60} />
            <div>
              <p className="text-lg font-bold">{annotatedImagesCount} / {totalImages}</p>
              <p className="text-xs text-gray-400">Images Annotated</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-gray-700/50 p-1 rounded-lg flex items-center gap-1">
          ...
          <div className="w-px h-6 bg-white/20 mx-1"></div>
           <button 
              ...
              disabled={!selectedAnnotationClass}
